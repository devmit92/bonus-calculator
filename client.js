...
$(document).ready(function() {
  for (let i = 0; i < newObjectsArray.length; i++ ) {
    ...
    let object = newObjectsArray[i];
    newDiv.append('<p>' + object.name + '</p>');
    newDiv.append('<p>' + object.bonusPercentage + '</p>');
    newDiv.append('<p>' + object.totalCompensation + '</p>');
    newDiv.append('<p>' + object.totalBonus + '</p>');
  }
...
function bonusCalculator(employeeArray){
  for(let i = 0 ; i < employeeArray.length; i++) {
    ...
    newObjectsArray.push(percentage(individualEmployee));
